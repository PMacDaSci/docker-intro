...
containers with Docker commands, and instead use a similar tool called Apptainer, Singularity or
...
run it using either of Docker, Singularity or Apptainer.
...
Apptainer is a container engine, like Docker.
However, unlike Docker, container images are typically stored as single files called `.sif` (Singularity Image Format).
For a number of reasons, Apptainer suits shared High Performance Computing (HPC) environments much better than Docker, so is valuable to learn if you work in these environments.

Apptainer and Singularity are closely related and largely command-compatible.
The main practical difference for this workshop is naming: modern systems often provide `apptainer`, while older documentation and installations may still use `singularity` for similar commands.
...
Since Apptainer containers have their own file format, if we have a Docker image we want to run, it first has to be converted.
We can do this using `apptainer pull`.
...
To run this container, we then use `apptainer run`:
...
If we want to modify the command run in the container, we have to use `apptainer exec`.
