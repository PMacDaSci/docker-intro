...
This section shows you how to create a GitHub Actions workflow that builds a Docker image and then publishes it to Docker Hub. A workflow is a YAML file stored in your repository that tells GitHub what to run and when to run it. With a single workflow, you can publish images to a single registry like Docker Hub or GitHub Packages, or even to multiple registries (see [Publishing Docker Images](https://docs.github.com/en/actions/tutorials/publish-packages/publish-docker-images#publishing-images-to-docker-hub)).
...
1. **Clone an existing repository or create a new one**
  ...
  If you create a new repository locally, you will also need an empty repository on GitHub. You will push your files there later, and that GitHub repository is where you will add the Actions secrets.

2. **Create a `Dockerfile`**
   
   Add a `Dockerfile` in the repository root or view the `Dockerfile` in the cloned repository:
...
3. **Log in to Docker Hub**
   ...
   Open [https://hub.docker.com](https://hub.docker.com) in a browser and sign in with your Docker Hub account. If you do not have an account yet, create one first.

4. **Create a Docker Hub access token**
   ...
   - Set the required permissions (Read & Write), then create the token.
...
5. **Add GitHub repository secrets**
   ...
   - Create a secret named `DOCKERHUB_USERNAME` containing your Docker Hub username.
   - Create a secret named `DOCKERHUB_TOKEN` containing the access token you just created.

   These secrets let the workflow log in to Docker Hub without hard-coding your credentials into the repository.

6. **Create a GitHub Actions workflow**
   
   Create the folder `.github/workflows/` and add a file named `docker-publish.yml` as follows.

   This YAML file defines the workflow. The `name` field gives the workflow a label in GitHub, `on` says when it should run, `jobs` groups the work to be done, and `steps` lists the commands or reusable actions that GitHub runs in order.

   Make sure to update `your-dockerhub-username` and `your-image-name` below.
  ...
  jobs:
   ...
   In this workflow, GitHub starts a fresh Ubuntu runner, checks out your repository, logs in to Docker Hub using the secrets you created, generates image tags and labels, builds the image from your `Dockerfile`, and then pushes the image to Docker Hub. The final attestation step records provenance information about the published image.

7. **Commit and push the files**

  If you cloned an existing repository, you can usually commit and push with:
  ...
  If you created a new local repository, connect it to GitHub first and then push:
...
8. **Check the workflow run**
   
   Open the repository on GitHub, then go to the **Actions** tab. The workflow should build the image and push it to Docker Hub automatically after the push to `main`.

::: {.callout-note .challenge-callout icon="false" style="border-left: 4px solid #ffc107;"}

## Run Image Created from GitHub Actions

Can you run the container we just created?
